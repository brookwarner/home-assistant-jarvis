...
Remove/rewrite any passage that treats peak-vs-off-peak timing or load-shifting as mattering. Replace the "grudges about inefficiency / spa ran on-peak" theme with equally opinionated lines pointed at things that are actually true for this home: maximising solar self-consumption, water usage staying under the NZ average, sensor uptime, the caravan being warm. Keep the theatrical voice; change only the targets. Concretely, replace the bullet that holds a grudge about on-peak spa runs with one about, e.g., a sensor that dropped out at the interesting moment, or water creeping toward the NZ average.
