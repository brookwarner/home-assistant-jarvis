...
These are quality/tuning problems in existing code, not missing features, and are tracked separately from the backlog of *feature* work. This work should ship first.
...
### 4. Fix dedup (repeats)
Replace truncated `_recent_alerts` with **real recent-message memory**: last N *full* sent messages with timestamps, fed to the proactive brief untruncated, with an explicit "you already told the owner this today; don't repeat it" instruction.
