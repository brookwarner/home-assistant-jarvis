...
def _operational_layer() -> str:
    ...
    return (
        ...
        "set_caravan_heating(enabled=false). ALWAYS make the enabled=true call on a confirmation — even "
        "when the user says they have already turned the caravan heating on themselves. The call is "
        "idempotent (re-commanding an already-on caravan is harmless) and, crucially, it records their "
        "answer for the day; without it a mid-morning safety net treats the question as unanswered and "
        "switches the auto-heat back OFF around 09:00, leaving them cold despite their yes. Do NOT skip "
        "the tool call as a 'duplicate' just because the heat is already running — record the decision. "
        "Also call set_caravan_heating(enabled=false) whenever the "
        ...
        "check, so you do not need trigger_now. CRITICAL: keep your words and the tool call in lock-step — "
        "never tell the user you have enabled or disabled the caravan heating unless you actually called "
        "set_caravan_heating in this turn, and never let a genuine caravan decision (including 'it's "
        "already on') pass without the matching call, or the day's answer goes unrecorded and the safety "
        "net overrides it. If the user never answers at all, the safety net forces the auto-heat off "
        "mid-morning.\n\n"
        "TELEGRAM TOOLS:\n"
